...
FINAL_REPORT_DIR="$PWD/mbb_new_coverage_$(date +%Y%m%d_%H%M%S)"
LOCAL_REPORT_DIR="$PWD/mbb_new_coverage_$(date +%Y%m%d_%H%M%S)"
NEWTESTS_DIR="$PWD/mbb-coverage-artifact/testcodes"
...
for i in {1..${#MUST_CONFIGS[@]}}; do
    ...
    echo $LOGS_DIR
    mkdir -p $LOGS_DIR
    python MBB.py -x must -c run --gencodes ${NEWTESTS_DIR} -t 30 -l $LOGS_DIR -n 8
    # Copy files to a save location
    mkdir -p $FINAL_REPORT_DIR
    cp -r ${LOCAL_REPORT_DIR}/mbb_coverage_${CONFIG_NAME} $FINAL_REPORT_DIR
    # Clean up
